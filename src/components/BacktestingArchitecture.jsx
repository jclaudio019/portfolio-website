...
export default function BacktestingArchitecture() {
    return (
        <figure
            ...
            className="mt-8 overflow-hidden border border-navy/10 bg-surface px-4 py-6 text-navy sm:px-6"
        >
            ...
            <div className="mt-5 space-y-6">
                {lanes.map((lane) => (
                    <section key={lane.label} aria-label={lane.label}>
                        <h3 className="text-sm font-semibold text-navy">{lane.label}</h3>
                        <ol className="mt-3 grid gap-2 sm:grid-flow-col sm:auto-cols-fr">
                            {lane.nodes.map((node, index) => (
                                <li key={node} className="relative border border-navy/20 bg-cream/40 px-3 py-3 text-sm">
                                    {node}
